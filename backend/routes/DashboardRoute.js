...
router.get("/:pdcId/showPDCDashboard", async (req, res) => {
  try {
    ...
    const rightCatcherChassisRails = await ChassisRailRightCatcherModel.find({
    ...
    const catcherRoofs = await RoofCatcherModel.find({
      _id: { $in: pdc.catcherRoofs },
    });
...
router.get(
  "/:chassisId/showRightCatcherChassisRailDashboard",
  async (req, res) => {
    try {
      const { chassisId } = req.params;
      ...
      // Find the PDC that contains the given Chassis Rail
      const pdc = await PDCModel.findOne({
        rightCatcherChassisRails: ChassisRail._id,
      }).populate("rightCatcherChassisRails");
      ...
      res.status(200).json({
        pdcId: pdc ? pdc.pdcId : null,
        workOrderId,
        rightCatcherChassisRails: pdc ? pdc.rightCatcherChassisRails : null,
        componentData,
      });
    } catch (error) {
      res
...
router.get("/:roofId/showCatcherRoofDashboard", async (req, res) => {
  try {
    const { roofId } = req.params;

    // Find Roof based on roofId
    const Roof = await RoofCatcherModel.findOne({
      roofId,
    });
    ...
    // Find the PDC that contains the given Roof
    const pdc = await PDCModel.findOne({
      catcherRoofs: Roof._id,
    }).populate("catcherRoofs");
    ...
    res.status(200).json({
      pdcId: pdc ? pdc.pdcId : null,
      workOrderId,
      catcherRoofs: pdc ? pdc.catcherRoofs : null,
      componentData,
    });
  } catch (error) {
    res.status(500).json({ message: `Error retrieving PDCs for CT Interface` });
  ...
  async (req, res) => {
    try {
      ...
      if (panel) {
        ...
        // Find the PDC that contains the specified CTInterface
        const PDC = await PDCModel.findOne({
          rightCatcherChassisRails: chassisRailObjectId,
        }).populate("rightCatcherChassisRails");
