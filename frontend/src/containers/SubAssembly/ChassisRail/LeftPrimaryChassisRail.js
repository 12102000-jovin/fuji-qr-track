...
import EditLeftPrimaryChassisRail from "./EditLeftPrimaryChassisRail";
...
const LeftPrimaryChassisRail = () => {
  const fetchChassisRailData_API =
    "http://localhost:3001/SubAssembly/LeftPrimaryChassisRail/getAllChassisRail";

  const deleteChassisRail_API =
    "http://localhost:3001/SubAssembly/LeftPrimaryChassisRail/deleteLeftPrimaryChassisRail/";
        ...
        if (selectedRow) {
          return {
            ...
            qrCodeData: JSON.stringify({
              ...
              leftPrimaryChassisRailId: selectedRow.chassisId,
            }),
          };
        }
        return null;
  ...
  const showQRCodes = (data, row) => {
    setQrCodeData(
      JSON.stringify({
        link: data.link,
        leftPrimaryChassisRailId: data.chassisId,
      })
    );
  ...
  const handleDownloadAllSelectedQR = async () => {
    ...
    zip.generateAsync({ type: "blob" }).then((content) => {
      ...
      a.href = url;
      a.download = `Selected-ChassisRail(Left)(Primary)`;
...
export default LeftPrimaryChassisRail;
