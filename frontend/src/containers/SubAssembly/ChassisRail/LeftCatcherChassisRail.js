...
import EditLeftCatcherChassisRail from "./EditLeftCatcherChassisRail";
...
const LeftCatcherChassisRail = () => {
  const fetchChassisRailData_API =
    "http://localhost:3001/SubAssembly/LeftCatcherChassisRail/getAllChassisRail";

  const deleteChassisRail_API =
    "http://localhost:3001/SubAssembly/LeftCatcherChassisRail/deleteLeftCatcherChassisRail/";
        ...
        if (selectedRow) {
          return {
            ...
            qrCodeData: JSON.stringify({
              ...
              leftCatcherChassisRailId: selectedRow.chassisId,
            }),
          };
        }
        return null;
  ...
  const showQRCodes = (data, row) => {
    setQrCodeData(
      JSON.stringify({
        link: data.link,
        leftCatcherChassisRailId: data.chassisId,
      })
    );
  ...
  const handleDownloadAllSelectedQR = async () => {
    ...
    zip.generateAsync({ type: "blob" }).then((content) => {
      ...
      a.href = url;
      a.download = `Selected-ChassisRail(Left)(Catcher)`;
...
export default LeftCatcherChassisRail;
