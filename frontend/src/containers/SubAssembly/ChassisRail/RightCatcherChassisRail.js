...
import EditRightCatcherChassisRail from "./EditRightCatcherChassisRail";
...
const RightCatcherChassisRail = () => {
  const fetchChassisRailData_API =
    "http://localhost:3001/SubAssembly/RightCatcherChassisRail/getAllChassisRail";

  const deleteChassisRail_API =
    "http://localhost:3001/SubAssembly/RightCatcherChassisRail/deleteRightCatcherChassisRail/";
        ...
        if (selectedRow) {
          return {
            ...
            qrCodeData: JSON.stringify({
              ...
              rightCatcherChassisRailId: selectedRow.chassisId,
            }),
          };
        }
        return null;
  ...
  const showQRCodes = (data, row) => {
    setQrCodeData(
      JSON.stringify({
        link: data.link,
        rightCatcherChassisRailId: data.chassisId,
      })
    );
  ...
  const handleDownloadAllSelectedQR = async () => {
    ...
    zip.generateAsync({ type: "blob" }).then((content) => {
      ...
      a.href = url;
      a.download = `Selected-ChassisRail(Right)(Catcher)`;
...
export default RightCatcherChassisRail;
