...
const EditRightCTInterface = ({ open, onClose, CTId }) => {
  ...
  const editCTInterface_API = `http://localhost:3001/SubAssembly/CTInterfaceRight/editCTInterfaceRight/${pdcId}/${CTId}`;
  const fetchDashboardCTInterfaceData_API = `http://localhost:3001/Dashboard/${CTId}/showRightCTInterfaceDashboard`;
  ...
  useEffect(() => {
    console.log("here is", CTId);
    console.log("here is", pdcId);
  }, []);
              ...
              <span className="flex items-center">
                <div className="flex">
                  <p className="text-xs text-white px-2 bg-red-500 rounded-full ml-3">
                    CT Interface (Right) has not been allocated to any PDC.{" "}
                  </p>
                  <a
...
export default EditRightCTInterface;
