...
const EditRoofCatcher = ({ open, onClose, roofId }) => {
  ...
  const [roofDisabled, setRoofDisabled] = useState(true);
  const [pdcId, setPDCId] = useState("");

  const fetchPDCData_API = "http://localhost:3001/PDC/getAllPDC";
  const fetchDashboardRoofData_API = `http://localhost:3001/Dashboard/${roofId}/showCatcherRoofDashboard`;
  const editRoof_API = `http://localhost:3001/SubAssembly/RoofCatcher/editRoof/${pdcId}/${roofId}`;

  useEffect(() => {
    // Fetch PDCId
    const fetchPDCId = async () => {
      try {
        const response = await axios.get(`${fetchDashboardRoofData_API}`);
  ...
  const handleRoofDisabled = () => {
    setRoofDisabled(false);
  };

  const handleEditRoof = async () => {
    console.log("Future PDC Id", selectedPDCId);
    console.log("Future Roof Id", roofIdToEdit);

    console.log("Current PDC Id", pdcId);
    console.log("Current Roof Id", roofId);

    setGeneralError("");

    try {
      const response = await axios.put(`${editRoof_API}`, {
        pdcToEdit: selectedPDCId,
        roofToEdit: roofIdToEdit,
      });

      if (response.status === 200) {
        console.log("Roof Edited Successfully");
        onClose();
      }
    } catch (error) {
      console.error("Error editing roof:", error);
              ...
              <span className="flex items-center">
                <div className="flex">
                  <p className="text-xs text-white px-2 bg-red-500 rounded-full ml-3">
                    Roof has not been allocated to any PDC.{" "}
                  </p>
                  <a
          ...
          <div className="flex items-center">
            <div className="relative w-full">
              <input
                ...
                value={roofIdToEdit}
                onChange={(e) => setRoofIdToEdit(e.target.value)}
...
export default EditRoofCatcher;
