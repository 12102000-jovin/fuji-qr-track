...
const Panel = () => {
  ...
  const handleDownloadSelectedRowQR = () => {
    const qrCodes = selectedRows
      .map((rowId) => {
        const selectedRow = panelData.find((row) => row._id === rowId);
        if (selectedRow) {
          return {
            ...
            {selectedRowsQRCodes.map((qrCode, index) => (
              ...
              >
                <div className="flex flex-col justify-center items-center">
                  <div ref={captureRef} id={`selectedQRCode-${qrCode.panelId}`}>
                    <ReactQRCode
