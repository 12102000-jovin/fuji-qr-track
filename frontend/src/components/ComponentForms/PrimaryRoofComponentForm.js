...
const PrimaryRoofComponentForm = (roofId) => {
  ...
  const AllocateRoof_API =
    "http://localhost:3001/Allocate/AllocatePrimaryRoofComponent";
  ...
  const handleAllocate = async () => {
    try {
      ...
      const response = await axios.post(AllocateRoof_API, {
        roofId: roofId.roofId,
        componentType: "Meter",
      ...
      if (response.status === 200) {
        ...
        setErrorMessage("");
        setSuccessfulMessage("Component Allocated to Roof successfully");
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="flex items-center">
              ...
              {isAllocateLoading
                ? "Allocating..."
                : `Allocate Components to ${roofId.roofId}`}
            </div>
          </button>
        </div>
      </div>
...
export default PrimaryRoofComponentForm;
