...
const ChassisRailRightCatcherComponentForm = (chassisId) => {
  ...
  const AllocateRightCatcherChassisRail_API =
    "http://localhost:3001/Allocate/AllocateRightCatcherChassisRailComponent";
  ...
  const handleAllocate = async () => {
    try {
      ...
      const response = await axios.post(AllocateRightCatcherChassisRail_API, {
        chassisId: chassisId.chassisId,
        componentType: "Meter",
      ...
      if (response.status === 200) {
        ...
        setSuccessfulMessage(
          "Component Allocated to Chassis Rail (Right) (Catcher) successfully"
        );
    ...
    <div>
      <div className="p-5 bg-signature text-white font-bold rounded-md">
        <div className="flex justify-start text-2xl font-black mb-5 items-center">
          ...
          <span className="text-xl text-white bg-blue-500 py-1 px-4 font-black rounded-full mr-2">
            R
          </span>
          Components
        </div>
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="flex items-center">
              ...
              {isAllocateLoading
                ? "Allocating..."
                : `Allocate Components to ${chassisId.chassisId}`}
            </div>
          </button>
        </div>
      </div>
...
export default ChassisRailRightCatcherComponentForm;
