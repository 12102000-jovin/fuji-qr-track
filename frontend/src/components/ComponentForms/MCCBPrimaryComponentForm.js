...
const MCCBPrimaryComponentForm = (MCCBId) => {
  ...
  const AllocateMCCB_API =
    "http://localhost:3001/Allocate/AllocateMCCBPrimaryComponent";
  ...
  const handleAllocate = async () => {
    try {
      ...
      const response = await axios.post(AllocateMCCB_API, {
        MCCBId: MCCBId.MCCBId,
        componentType: "Meter",
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="flex items-center">
              ...
              {isAllocateLoading
                ? "Allocating..."
                : `Allocate Components to ${MCCBId.MCCBId}`}
            </div>
          </button>
        </div>
      </div>
...
export default MCCBPrimaryComponentForm;
