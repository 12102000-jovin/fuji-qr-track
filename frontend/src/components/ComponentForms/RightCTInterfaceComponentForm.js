...
const RightCTInterfaceComponentForm = (CTId) => {
  ...
  const AllocateRightCTInterface_API =
    "http://localhost:3001/Allocate/AllocateRightCTInterfaceComponent";
  ...
  const handleAllocate = async () => {
    console.log("Here", CTId);
    try {
      setIsAllocateLoading(true);
      const response = await axios.post(AllocateRightCTInterface_API, {
        CTId: CTId.CTId,
        componentType: "Meter",
      ...
      if (response.status === 200) {
        ...
        setSuccessfulMessage(
          "Component Allocated to CT Interface (Right) successfully"
        );
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="flex items-center">
              ...
              {isAllocateLoading
                ? "Allocating..."
                : `Allocate Components to ${CTId.CTId}`}
            </div>
          </button>
        </div>
      </div>
...
export default RightCTInterfaceComponentForm;
