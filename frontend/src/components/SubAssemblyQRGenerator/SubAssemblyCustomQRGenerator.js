...
const SubAssemblyQRGenerator = () => {
  ...
  useEffect(() => {
    if (numQR === "1") {
      setExpectedLeftCatcherChassisRailRange(
        `Expected Output: CHR${formatId(Number(startNum))}L-C`
      );
    } else if (numQR > 1 && numQR !== "1") {
      setExpectedLeftCatcherChassisRailRange(
        ` Expected Output: CHR${formatId(Number(startNum))}L-C - CHR${formatId(
          Number(startNum) + Number(numQR) - 1
        )}L-C`
      );
    } else {
  ...
  // ==================================== C H A S S I S  R A I L (L E F T) (R I G H T) =======================================
  ...
  useEffect(() => {
    if (numQR === "1") {
      setExpectedRightCatcherChassisRailRange(
        `Expected Output: CHR${formatId(Number(startNum))}R-C`
      );
    } else if (numQR > 1 && numQR !== "1") {
      setExpectedRightCatcherChassisRailRange(
        ` Expected Output: CHR${formatId(Number(startNum))}R-C - CHR${formatId(
          Number(startNum) + Number(numQR) - 1
        )}R-C`
      );
    } else {
  ...
  useEffect(() => {
    if (numQR === "1") {
      setExpectedCatcherRoofRange(
        `Expected Output: ROOF${formatId(Number(startNum))}-C`
      );
    } else if (numQR > 1 && numQR !== "1") {
      setExpectedCatcherRoofRange(
        ` Expected Output: ROOF${formatId(Number(startNum))}-C - ROOF${formatId(
          Number(startNum) + Number(numQR) - 1
        )}-C`
      );
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSubAssemblyType === "Panel") {
      ...
      const LeftPrimaryChassisRails = Array.from(
        { length: numQR },
        (_, index) => {
          const newLeftPrimaryChassisRailId = formatId(
            Number(startNum) + index
          );
          return {
            link: `${linkFormat}${applicationPortNumber}/Dashboard/ChassisRail/CHR${newLeftPrimaryChassisRailId}L-P`,
            generatedDate: moment()
              .tz("Australia/Sydney")
              .format("YYYY-MM-DD HH:mm:ss"),
            chassisId: `CHR${newLeftPrimaryChassisRailId}L-P`,
          };
        }
      );
      ...
      try {
        ...
        setQRcode(() => {
          const newQRCodes = response.data.map((qrcode, index) => ({
            ...qrcode,
            chassisId: `CHR${formatId(Number(startNum) + index)}L-P`,
          }));
          return newQRCodes;
        });
      ...
      // ======================================= C H A S S I S (R I G H T) (P R I M A R Y) =======================================
      ...
      const LeftCatcherChassisRails = Array.from(
        { length: numQR },
        (_, index) => {
          const newLeftCatcherChassisRailId = formatId(
            Number(startNum) + index
          );
          return {
            link: `${linkFormat}${applicationPortNumber}/Dashboard/ChassisRail/CHR${newLeftCatcherChassisRailId}L-C`,
            generatedDate: moment()
              .tz("Australia/Sydney")
              .format("YYYY-MM-DD HH:mm:ss"),
            chassisId: `CHR${newLeftCatcherChassisRailId}L-C`,
          };
        }
      );
      ...
      try {
        ...
        setQRcode(() => {
          const newQRCodes = response.data.map((qrcode, index) => ({
            ...qrcode,
            chassisId: `CHR${formatId(Number(startNum) + index)}L-C`,
          }));
          return newQRCodes;
        });
      ...
      const RightCatcherChassisRails = Array.from(
        { length: numQR },
        (_, index) => {
          const newRightCatcherChassisRailId = formatId(
            Number(startNum) + index
          );
          return {
            ...
            generatedDate: moment()
              .tz("Australia/Sydney")
              .format("YYYY-MM-DD HH:mm:ss"),
            chassisId: `CHR${newRightCatcherChassisRailId}R-C`,
          };
        }
      );
      ...
      try {
        ...
        setQRcode(() => {
          const newQRCodes = response.data.map((qrcode, index) => ({
            ...qrcode,
            chassisId: `CHR${formatId(Number(startNum) + index)}R-C`,
          }));
          return newQRCodes;
        });
      ...
      const CatcherRoofs = Array.from({ length: numQR }, (_, index) => {
        const newCatcherRoofId = formatId(Number(startNum) + index);
        return {
      ...
      try {
        ...
        setQRcode(() => {
          const newQRCodes = response.data.map((qrcode, index) => ({
            ...qrcode,
            roofId: `ROOF${formatId(Number(startNum) + index)}-C`,
          }));
          return newQRCodes;
        });
  ...
  useEffect(() => {
    ...
    if (currentUrl.includes("Panel")) {
      ...
      // Set the selected value of the dropdown to "ChassisRailLeftCatcher"
