...
const PDCCustomQRGenerator = () => {
  ...
  useEffect(() => {
    if (numQR === "1") {
      setExpectedRange(`Expected Output: PDC${formatPDCId(Number(startNum))}`);
    } else if (numQR > 1 && numQR !== "1") {
      setExpectedRange(
        ` Expected Output: PDC${formatPDCId(
          Number(startNum)
        )} - PDC${formatPDCId(Number(startNum) + Number(numQR) - 1)}`
      );
    } else {
      setExpectedRange("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const PDCs = Array.from({ length: numQR }, (_, index) => {
      console.log("Latest PDC:", startNum);

      const newPDCId = formatPDCId(Number(startNum) + index);
      return {
...
export default PDCCustomQRGenerator;
